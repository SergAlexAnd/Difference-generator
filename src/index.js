...
const generateKeys = (before, after) => {
  ...
  const addedKeys = _.difference(afterKeys, beforeKeys).map(key => ({ type: 'plus', key, value: after[key] }));
  const removedKeys = _.difference(beforeKeys, afterKeys).map(key => ({ type: 'minus', key, value: before[key] }));
